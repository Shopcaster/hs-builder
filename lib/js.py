...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Compile js')
    parser.add_argument('-v', '--verbose', default=False, dest='v',
                        action='store_true', help='verbose output')
    parser.add_argument('-c', '--compiled', default=False, action='store_true',
                        help='Compile the JS')
    parser.add_argument('-j', '--js_dir', help='The location of the JS.',
                        required=True)
    args = parser.parse_args()

    if (args.compiled):
        print 'TODO: support compilation'
    else:
        jsc = JavaScriptCompiler(args.js_dir, DummyMinifier(), v=args.v, recurse=True)
        jsc.resolve_dependencies()
        print json.dumps(jsc.file_list)
